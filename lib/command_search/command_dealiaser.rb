module CommandSearch
  module CommandDealiaser
    ...
    def self.unnest_unaliased(node, aliases)
      ...
      return node if type == :colon && aliases[values.first] # look into this lines test coverage and all
      return node if type == :compare && (values & aliases.keys).any? # look into this lines test coverage and all
      str_values = values.join(node[:nest_op])
      { type: :str, value: str_values }
    end

    def self.dealias(ast, aliases)
      ast.flat_map do |x| # look into map!
